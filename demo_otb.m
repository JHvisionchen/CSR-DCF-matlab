...
tracker_path = '/media/cjh/cvpaper/git/CSR-DCF-matlab';
% add paths
...
img_dir = dir(fullfile(base_path, 'img/*.jpg'));

% initialize bounding box - [x,y,width, height]
gt = read_vot_regions(fullfile(base_path, 'groundtruth_rect.txt'));
...
while frame <= numel(img_dir),  % tracking loop
	% read frame
    impath = fullfile(base_path, 'img', img_dir(frame).name);
    img = imread(impath);
